...
namespace EsgynDB.Data
{
    ...
    /// </summary>
    internal class EsgynDBMetaData
    {
        ...
        public EsgynDBMetaData(EsgynDBConnection conn)
        {
            _conn = conn;
            _dataset = null;
        ...
        private void Init()
        {
            ...
            if (_dataset == null)
            {
                ...
                _dataset.ReadXml(Assembly.GetExecutingAssembly().GetManifestResourceStream("EsgynDB.Data.Resources.Metadata.xml"), XmlReadMode.ReadSchema);
            }
        }
        ...
        public DataTable GetSchema(string collectionName, string[] restrictionValues)
        {
            if (EsgynDBTrace.IsInternalEnabled)
            {
                ...
                EsgynDBTrace.Trace(this._conn, TraceLevel.Internal, p);
            }
            ...
            switch (collectionName)
            {
                ...
                default:
                    EsgynDBException.ThrowException(this._conn, new InvalidOperationException("Invalid Collection Name: " + collectionName));
                    break;
            }
        ...
        private DataTable ExecuteQuery(string query)
        {
            if (EsgynDBTrace.IsInternalEnabled)
            {
                EsgynDBTrace.Trace(this._conn, TraceLevel.Internal, query);
            }

            DataTable dt;

            using (EsgynDBCommand cmd = this._conn.CreateCommand())
            {
                cmd.CommandText = query;
                using (EsgynDBDataAdapter da = new EsgynDBDataAdapter(cmd))
                {
                    dt = new DataTable();
                    da.Fill(dt);
                }
        ...
        private void CopyGenericRestrictions(string[] source, int sourceOffset, object[] dest, int destOffset, int count)
        {
            for (int i = 0; i < count; i++, sourceOffset++)
            {
                ...
                {
                    dest[destOffset++] = EsgynDBUtility.FindWildCards.IsMatch(source[sourceOffset]) ? "LIKE" : "=";
