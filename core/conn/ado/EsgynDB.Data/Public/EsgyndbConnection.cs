namespace EsgynDB.Data
{
    ...
    /// </summary>
    public sealed class EsgynDBConnection : DbConnection, IDbConnection
    {
        ...
        private EsgynDBMetaData _metaData;
        private EsgynDBSecurity _security;
        ...
        //lock for access to DCSMaster server
        internal static readonly List<String> asSrvrLock = new List<String>();

        static EsgynDBConnection()
        {
            EsgynDBConnection._connPools = new Dictionary<string, ConnectionPool>();
        }

        /// <summary>
        /// Initializes a new instance of the EsgynDBConnection class.
        /// </summary>
        public EsgynDBConnection()
            : this(string.Empty)
        {
        }
        ...
        public EsgynDBConnection(string connectionString)
        {
            ...
            this.ConnectionStringBuilder = new EsgynDBConnectionStringBuilder(connectionString);
            this.Network = new EsgynDBNetwork(this);
            this.Commands = new List<EsgynDBCommand>();
            this._metaData = new EsgynDBMetaData(this);
        }

        /// <summary>
        /// Event for viewing status and warning messages during operation.
        /// </summary>
        public event EsgynDBInfoMessageEventHandler InfoMessage;
        ...
        public override string Database
        {
            get
            {
                if (this._database == null)
                {
                    ...
                    using (EsgynDBCommand cmd = this.CreateCommand())
                    {
        ...
        /// <summary>
        /// Gets the EsgynDB server version.
        /// </summary>
        ...
        public override ConnectionState State
        {
            get
            {
                ...
                {
                    EsgynDBException.ThrowException(this, new CommunicationsFailureException("Idle Timeout"));
                }
                return this._state;
            }
        }
        ...
        internal EsgynDBTransaction Transaction
        {
            get;
            set;
        }

        internal List<EsgynDBCommand> Commands // a list of the currently allocated commands
        {
            get;
            set;
        }
        ...
        internal EsgynDBConnectionStringBuilder ConnectionStringBuilder
        {
            get;
            set;
        }
        ...
        /// <summary>
        /// Provides access to the default EsgynDBFactory instance.
        /// </summary>
        protected override DbProviderFactory DbProviderFactory
        {
            get
            {
                return EsgynDBFactory.Instance;
            }
        }

        /// <summary>
        /// Releases the resources used by the EsgynDBConnection.
        /// </summary>
        public new void Dispose()
        {
            if (EsgynDBTrace.IsPublicEnabled)
            {
                EsgynDBTrace.Trace(this, TraceLevel.Public);
            }
        ...
        public new EsgynDBTransaction BeginTransaction()
        {
        ...
        /// <returns>An object representing the new transaction.</returns>
        public new EsgynDBTransaction BeginTransaction(IsolationLevel isolationLevel)
        {
            if (EsgynDBTrace.IsPublicEnabled)
            {
                EsgynDBTrace.Trace(this, TraceLevel.Public, isolationLevel);
            }
            ...
            this.Transaction = new EsgynDBTransaction(this, isolationLevel);
        ...
        public override void ChangeDatabase(string databaseName)
        {
            if (EsgynDBTrace.IsPublicEnabled)
            {
                EsgynDBTrace.Trace(this, TraceLevel.Public, databaseName);
            }
                ...
                try
                {
                    ...
                    if (this.State != ConnectionState.Closed || forceClose)
                    {
                        ...
                        if (this.ConnectionStringBuilder.MaxPoolSize > 0)
                        {
                            ...
                            if (!isFreeStmtErr && !this.Network.IsClosed && !this.Network.isIdleTimeout)
                            {
                        ...
                        if ((forceClose || this.ConnectionStringBuilder.MaxPoolSize <= 0) && !isDisconnectErr && !this.Network.IsClosed && !this.Network.isIdleTimeout)
                        {
        ...
        /// <summary>
        /// Creates a new <code>EsgyndbCommand</code> assocaited with this connection.
        /// </summary>
        /// <returns>The <code>EsgyndbCommand</code> object created</returns>
        public new EsgynDBCommand CreateCommand()
        {
            if (EsgynDBTrace.IsPublicEnabled)
            {
                EsgynDBTrace.Trace(this, TraceLevel.Public);
            ...
            EsgynDBCommand cmd = new EsgynDBCommand(string.Empty, this, this.Transaction);
        ...
        /// <summary>
        /// Creates a new <code>EsgyndbCommand</code> with the given label assocaited with this connection.
        /// </summary>
        /// <param name="cmdLabel">The label to be associated with the newly created EsgyndbCommand.</param>
        /// <returns>The <code>EsgyndbCommand</code> object created</returns>
        public new EsgynDBCommand CreateCommand(string cmdLabel)
        {
            if (EsgynDBTrace.IsPublicEnabled)
            {
                EsgynDBTrace.Trace(this, TraceLevel.Public, cmdLabel);
            }

            EsgynDBCommand cmd = new EsgynDBCommand(string.Empty, this, this.Transaction, cmdLabel);
            this.Commands.Add(cmd);
        ...
        /// <summary>
        /// Creates and returns a EsgyndbCommand object associated with the EsgynDBConnection.
        /// </summary>
        ...
        /// <summary>
        /// Opens the connection to the EsgynDB Server.
        /// </summary>
            ...
            lock (asSrvrLock)
            {
                ...
                if (this.ConnectionStringBuilder.MaxPoolSize > 0)
                {
                    if (!EsgynDBConnection._connPools.ContainsKey(connectionString))
                    {
                        ...
                        EsgynDBConnection._connPools.Add(connectionString, pool);
                    }
                    else
                    {
        ...
        internal void Cancel()
        {
            if (!this.ConnectionStringBuilder.IgnoreCancel)
            {
                ...
                EsgynDBNetwork net = new EsgynDBNetwork(this);
        ...
        internal void SendInfoMessage(object sender, EsgynDBException e)
        {
            if (this.InfoMessage != null && e.Warnings.Count > 0)
            {
                this.InfoMessage(sender, new EsgynDBInfoMessageEventArgs(e.Warnings));
            }
        }
        ...
        internal bool RemoveCommand(EsgynDBCommand cmd)
        {
            if (EsgynDBTrace.IsInternalEnabled)
            {
                EsgynDBTrace.Trace(this, TraceLevel.Internal, cmd);
            }
        ...
        private GetObjRefReply GetObjRef()
        {
                ...
                try
                {
                    ...
                    switch (reply.error)
                    {
                        ...
                        case GetObjRefError.ASNoSrvrHdl:
                            throw new EsgynDBException(EsgynDBMessage.NoServerHandle);
                        case GetObjRefError.DSNotAvailable:
                            throw new EsgynDBException(EsgynDBMessage.DSNotAvailable);
                        case GetObjRefError.PortNotAvailable:
                            throw new EsgynDBException(EsgynDBMessage.PortNotAvailable);
            ...
            {
                this.SendInfoMessage(this, new EsgynDBException(EsgynDBMessage.ConnectedToDefaultDS, null, reply.datasource));
            }
            ...
            for (int i = 0; !success && i < retryCount; i++)
            {
                ...
                switch (reply.error)
                {
                    ...
                    case InitDialogueError.InvalidUser:
                        ...
                        {
                            e = new EsgynDBException(reply.errorDesc);
                            this.SendInfoMessage(this, e);
                            throw e;
                        }
        ...
        {
            EsgynDBConnectionStringBuilder b = this.ConnectionStringBuilder;
        ...
        private void SecureLogin(GetObjRefReply objRefReply)
        {
            ...
            try
            {
                this._security = new EsgynDBSecurity(
                    this,
                    null,
                    null,
                    objRefReply.cluster,
