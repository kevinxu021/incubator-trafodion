...
namespace LZF.NET
{
	...
	{
		// CRC32 data & function
		static readonly UInt32 []crc_32_tab = new UInt32[256]
		{
		...
		static public UInt32 crc32(UInt32 OldCRC,byte NewData) 
		{
		...
		static readonly UInt32 HLOG=14;
		static readonly UInt32 HSIZE=(1<<14);
		...
		* it works ;)
		*/
		static readonly UInt32 MAX_LIT=(1 <<  5);
		static readonly UInt32 MAX_OFF=(1 << 13);
		static readonly UInt32 MAX_REF=((1 <<  8) + (1 << 3));

		static UInt32 FRST(byte[] Array,UInt32 ptr) 
		{
			return (UInt32)(((Array[ptr]) << 8) | Array[ptr+1]);
		}

		static UInt32 NEXT(UInt32 v,byte[] Array,UInt32 ptr)
		{
			return ((v) << 8) | Array[ptr+2];
		}

		static UInt32 IDX(UInt32 h) 
		{
		...
		*/
		...
		static public int lzf_compress (byte[] in_data, int in_len,byte[] out_data, int out_len)
		{
			int c;
		...
		/// LZF Decompressor
		/// </summary>
		static public int lzf_decompress ( byte[] in_data, int in_len, byte[] out_data, int out_len)
		{
			UInt32 iidx=0;
			UInt32 oidx=0;
