...
namespace HP.Data.Neoview
{
    ...
    /// </summary>
    internal class NeoviewNetwork : IDisposable
    {
        private TcpClient _tcpClient;
        private NetworkStream _networkStream;
        private NeoviewConnection _connection;
        private NeoviewEncoder _encoder;
        ...
        internal NeoviewConnection Connection
        {
        ...
        internal NeoviewEncoder Encoder
        {
        ...
        internal NeoviewNetwork(NeoviewConnection conn)
        {
            if (NeoviewTrace.IsInternalEnabled)
            {
                NeoviewTrace.Trace(conn, TraceLevel.Internal);
            }
            ...
            this._encoder = new NeoviewEncoder(this._byteOrder);
        }
        ...
        internal void OpenIO(string server, int port)
        {
            if (NeoviewTrace.IsInternalEnabled)
            {
                NeoviewTrace.Trace(this._connection, TraceLevel.Internal, server, port);
            }
            ...
            catch (Exception e)
            {
                ...
                NeoviewException.ThrowException(this._connection, new CommunicationsFailureException(e));
            }
        ...
        internal void CloseIO()
        {
            if (NeoviewTrace.IsInternalEnabled)
            {
                NeoviewTrace.Trace(this._connection, TraceLevel.Internal);
            }
        ...
        internal void DoIO(INetworkMessage message, INetworkReply reply, OperationId id, DataStream fetchDs, BufferType fetchType, int IOtimeout)
        {
            if (NeoviewTrace.IsInternalEnabled)
            {
                NeoviewTrace.Trace(this._connection, TraceLevel.Internal, id);
            }
            ...
            if (this._isClosed)
            {
                string msg = NeoviewResources.FormatMessage(NeoviewMessage.InvalidConnectionState, this._connection.State);
                NeoviewException.ThrowException(this._connection, new InvalidOperationException(msg));
            }
            ...
            try
            {
                ...
                if (NeoviewTrace.IsNetworkEnabled)
                {
                    NeoviewTrace.Trace(this._connection, TraceLevel.Network, id, "SEND", this.ToHexString(this._ds.Buffer, length));
                }
                // ***** BEGIN COMPRESSION *****
                ...
                if (NeoviewTrace.IsNetworkEnabled)
                {
                    NeoviewTrace.Trace(this._connection, TraceLevel.Network, id, "RECV", this.ToHexString(this._ds.Buffer, length));
                }
        ...
        private void Recv(int length, int IOTimeout)
        {
            ...
            if (totalRead < length)
            {
                NeoviewException.ThrowException(this._connection, new CommunicationsFailureException("Socket unexpectedly closed: Server Process: " + this.Connection.RemoteProcess + " , " + this.Connection.RemoteAddress + ":" + this.Connection.RemotePort + " TotalToRead: " + length + " TotalBytesRead: " + totalRead + " CurrentBytesRead: " + read ));
            }
        }
    }
}
