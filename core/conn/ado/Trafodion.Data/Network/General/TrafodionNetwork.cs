namespace Trafodion.Data
{
    ...
    internal class HPDbNetwork : IDisposable
    {
        ...
        private TrafDbConnection _connection;
        private TrafDbEncoder _encoder;
        ...
        internal TrafDbConnection Connection
        {
        ...
        internal HPDbNetwork(TrafDbConnection conn)
        {
            ...
            {
                TrafDbTrace.Trace(conn, TraceLevel.Internal);
            }
            ...
            this._encoder = new TrafDbEncoder(this._byteOrder);
        }
        ...
        internal void OpenIO(string server, int port)
        {
            if (TrafDbTrace.IsInternalEnabled)
            {
                TrafDbTrace.Trace(this._connection, TraceLevel.Internal, server, port);
            }
            ...
            catch (Exception e)
            {
                ...
                TrafDbException.ThrowException(this._connection, new CommunicationsFailureException(e));
            }
        ...
        internal void CloseIO()
        {
            if (TrafDbTrace.IsInternalEnabled)
            {
                TrafDbTrace.Trace(this._connection, TraceLevel.Internal);
            }
        ...
        internal void DoIO(INetworkMessage message, INetworkReply reply, OperationId id, DataStream fetchDs, BufferType fetchType, int IOtimeout)
        {
            if (TrafDbTrace.IsInternalEnabled)
            {
                TrafDbTrace.Trace(this._connection, TraceLevel.Internal, id);
            }
            ...
            if (this._isClosed)
            {
                string msg = TrafDbResources.FormatMessage(TrafDbMessage.InvalidConnectionState, this._connection.State);
                TrafDbException.ThrowException(this._connection, new InvalidOperationException(msg));
            }
            ...
            try
            {
                ...
                if (TrafDbTrace.IsNetworkEnabled)
                {
                    TrafDbTrace.Trace(this._connection, TraceLevel.Network, id, "SEND", this.ToHexString(this._ds.Buffer, length));
                }
                // ***** BEGIN COMPRESSION *****
                ...
                if (TrafDbTrace.IsNetworkEnabled)
                {
                    TrafDbTrace.Trace(this._connection, TraceLevel.Network, id, "RECV", this.ToHexString(this._ds.Buffer, length));
                }
        ...
        private void Recv(int length, int IOTimeout)
        {
            ...
            if (totalRead < length)
            {
                TrafDbException.ThrowException(this._connection, new CommunicationsFailureException("Socket unexpectedly closed"));
            }
        }
        /// <summary>
        /// Reset IdleTimer
        ...
        private void IdleTimerReset()
        {
            ...
            {
                TrafDbException.ThrowException(this._connection, new CommunicationsFailureException("Idle Timeout"));
            }
            this._timer.Dispose();
        }
        /// <summary>
        ...
        {
            TrafDbConnection connection = (TrafDbConnection)source;
