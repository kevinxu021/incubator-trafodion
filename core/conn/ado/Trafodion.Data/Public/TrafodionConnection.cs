namespace Trafodion.Data
{
    ...
    public sealed class TrafDbConnection : DbConnection, IDbConnection
    {
        ...
        static TrafDbConnection()
        {
            TrafDbConnection._connPools = new Dictionary<string, ConnectionPool>();
        }
        ...
        public override string ConnectionString
        {
            ...
            set
            {
                ...
                {
                    string msg = TrafDbResources.FormatMessage(TrafDbMessage.InvalidConnectionState, this._state);
                    TrafDbException.ThrowException(this, new InvalidOperationException(msg));
                }
        ...
        /// <summary>
        /// Provides access to the default TrafDbFactory instance.
        /// </summary>
        protected override DbProviderFactory DbProviderFactory
        {
            get
            {
                return TrafDbFactory.Instance;
            }
        }
        ...
        public new void Dispose()
        {
            if (TrafDbTrace.IsPublicEnabled)
            {
                TrafDbTrace.Trace(this, TraceLevel.Public);
            }
        ...
        public new TrafDbTransaction BeginTransaction()
        {
        ...
        /// <returns>An object representing the new transaction.</returns>
        public new TrafDbTransaction BeginTransaction(IsolationLevel isolationLevel)
        {
            if (TrafDbTrace.IsPublicEnabled)
            {
                TrafDbTrace.Trace(this, TraceLevel.Public, isolationLevel);
            }
            ...
            this.Transaction = new TrafDbTransaction(this, isolationLevel);
        ...
        public override void ChangeDatabase(string databaseName)
        {
            if (TrafDbTrace.IsPublicEnabled)
            {
                TrafDbTrace.Trace(this, TraceLevel.Public, databaseName);
            }
        ...
        internal void Close(bool forceClose)
        {
            if (TrafDbTrace.IsPublicEnabled)
            {
                TrafDbTrace.Trace(this, TraceLevel.Public, forceClose);
            }
            ...
            try
            {
                ...
                lock (this.dataAccessLock)
                {
                    ...
                    if (this.State != ConnectionState.Closed || forceClose)
                    {
                        ...
                        {
                            TrafDbConnection._connPools[this.ConnectionStringBuilder.ConnectionString].AddPooledConnection(this);
        ...
        public new TrafDbCommand CreateCommand()
        {
            ...
            TrafDbCommand cmd = new TrafDbCommand(string.Empty, this, this.Transaction);
        ...
        public new TrafDbCommand CreateCommand(string cmdLabel)
        {
            if (TrafDbTrace.IsPublicEnabled)
            {
                TrafDbTrace.Trace(this, TraceLevel.Public, cmdLabel);
            }

            TrafDbCommand cmd = new TrafDbCommand(string.Empty, this, this.Transaction, cmdLabel);
            this.Commands.Add(cmd);
        ...
        public override void Open()
        {
            ...
            ConnectionPool pool;
            Monitor.Enter(TrafDbConnection._connPools);
            if (this.ConnectionStringBuilder.MaxPoolSize > 0)
            {
                if (!TrafDbConnection._connPools.ContainsKey(connectionString))
                {
                    pool = new ConnectionPool(this.ConnectionStringBuilder);
                    TrafDbConnection._connPools.Add(connectionString, pool);
                }
                else
                {
                    pool = TrafDbConnection._connPools[connectionString];
                }

                TrafDbConnection conn = pool.GetPooledConnection();
                if (conn != null)
                {
                    CopyProperties(conn);
                    initialized = true;
        ...
        internal void SendInfoMessage(object sender, TrafDbException e)
        {
            if (this.InfoMessage != null && e.Warnings.Count > 0)
            {
                this.InfoMessage(sender, new TrafDbInfoMessageEventArgs(e.Warnings));
            }
        }
        ...
        internal bool RemoveCommand(TrafDbCommand cmd)
        {
            ...
            else if (this._state == ConnectionState.Open)
            {
                ...
                switch (reply.error)
                {
                    ...
                    case CloseError.SqlError:
                        TrafDbException e = new TrafDbException(reply.errorDesc);
                    ...
                    default:
                        throw new TrafDbException(TrafDbMessage.InternalError, null, "FreeCommand", reply.error.ToString() + " " + reply.errorDetail + " " + reply.errorText);
                }
            }
        ...
        private GetObjRefReply GetObjRef()
        {
                ...
                try
                {
                    ...
                    switch (reply.error)
                    {
                        ...
                        case GetObjRefError.ASNoSrvrHdl:
                            throw new TrafDbException(TrafDbMessage.NoServerHandle);
                        case GetObjRefError.DSNotAvailable:
                            throw new TrafDbException(TrafDbMessage.DSNotAvailable);
                        case GetObjRefError.PortNotAvailable:
                            throw new TrafDbException(TrafDbMessage.PortNotAvailable);
                        ...
                        default:
                            throw new TrafDbException(TrafDbMessage.InternalError, "GetObjRef", reply.error.ToString() + " " + reply.errorDetail + " " + reply.errorText);
                    }
                }
                catch (Exception e)
                {
            ...
            {
                this.SendInfoMessage(this, new TrafDbException(TrafDbMessage.ConnectedToDefaultDS, null, reply.datasource));
            }
            ...
            for (int i = 0; !success && i < retryCount; i++)
            {
                ...
                switch (reply.error)
                {
                    ...
                    case InitDialogueError.InvalidUser:
                        ...
                        {
                            e = new TrafDbException(reply.errorDesc);
                            this.SendInfoMessage(this, e);
                            throw e;
                        }
                    ...
                    default:
                        throw new TrafDbException(TrafDbMessage.InternalError, null, "InitDiag", reply.error.ToString() + " " + reply.errorDetail + " " + reply.errorText);
                }
            }
        ...
        private void CopyProperties(TrafDbConnection conn)
        {
