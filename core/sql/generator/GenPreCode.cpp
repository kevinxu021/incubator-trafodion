			       ...
			       ValueIdSet &pulledNewInputs)
{
  ...
  if (useMinMaxOpt) {
    ...
    const PartitioningFunction *pf =
      child(1)->getPhysicalProperty()->getPartitioningFunction();
    ...
    NABoolean hashTableIsNotPartitioned =
      (pf==NULL || pf->getCountOfPartitions() == 1 ||
       pf->castToReplicateViaBroadcastPartitioningFunction());
    ...
    for (ValueId valId = getEquiJoinPredicates().init();
	 ...
	 getEquiJoinPredicates().advance(valId)) {
      ...
      if(mmType) {
        ...
        generator->addMinMaxVals(itemExpr->getValueId(),
                                 minVal->getValueId(),
                                 maxVal->getValueId(),
  ...
  for (CollIndex i = getStartMinMaxIndex(); i < getEndMinMaxIndex(); i++)
    {
      if (generator->isMinMaxKeyUsed(i))
        {
          minMaxCols_.insert(generator->getMinMaxKey(i));
              ...
              if (members.contains(minMaxKeyCol)) {
                ...
                if(!getSearchKey() || !getSearchKey()->isBeginKeyExclusive()) {
                  ...
                  ValueId hashJoinBeg = (ascKey ?
                                         generator->getMinVal(keyIdx) :
                                         generator->getMaxVal(keyIdx));
     ...
     ValueIdSet& availableValues)
{
  ...
  for (CollIndex i = 0; mmKeyVec.nextUsed(i); i++) {
    ...
    if (partitionColumns.contains(mmKeyId)) {
          ...
          minMaxValues += generator->getMinVal(i);
          minMaxValues += generator->getMaxVal(i);
			       ...
			       ValueIdSet &pulledNewInputs)
{
  ...
  // block the candidate values for min/max optimization that could produce
  // different min/max values for different instances from going
  // below this Exchange.  Restore them upon return from
  // preCodeGen'ing the child.
  NABitVector savedState;

  if(isEspExchange())
    generator->disablePartitionedMinMaxKeys(savedState);
  ...
  if(isEspExchange())
    generator->reenablePartitionedMinMaxKeys(savedState);
