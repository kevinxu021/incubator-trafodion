...
const char OsimHHDFSORCFileStats::elemName[] = TAG_HHDFSORCFILESTATS;
...
void OptimizerSimulator::dropObjects()
{
   ...
   if(!tables.good())
   {
       ...
       errMsg += logFilePaths_[CREATE_TABLE_DDLS];
       OsimLogException(errMsg.data(), __FILE__, __LINE__).throwException();
   }
...
void OptimizerSimulator::loadDDLs()
{
    ...
    {
        NAString errMsg = "Error open ";
        errMsg += logFilePaths_[CREATE_SCHEMA_DDLS];
        OsimLogException(errMsg.data(), __FILE__, __LINE__).throwException();
    }    
    //ignore first 2 lines
...
void OptimizerSimulator::loadHiveDDLs()
{
    ...
    {
         CmpCommon::diags()->mergeAfter(*(cliInterface_->getDiagsArea()));
         NAString errMsg = "create external table: " ;
         errMsg += std::to_string((long long)(retcode)).c_str();
         errMsg += statement;
         OsimLogException(errMsg.data(), __FILE__, __LINE__).throwException();
    }
    ...
    for(iterator.getNext(qualName, tableUID); qualName && tableUID; iterator.getNext(qualName, tableUID))
    {
       ...
       int count = 0;
       //read twice, one for hive table, one for external table
        while(readHiveStmt(hiveCreateTableSql, statement, comment) && count++ < 2)
        {
             if(comment.length() > 0)
                 debugMessage("%s\n", comment.data());
             if(statement.length() > 0){
                 if(((int)statement.index("CREATE EXTERNAL TABLE")) >= 0)
                 {     
                       //drop external table and hive table with the same name;
                       ...
                       {
                              CmpCommon::diags()->mergeAfter(*(cliInterface_->getDiagsArea()));
                              NAString errMsg = "drop external table: " ;
                              errMsg += std::to_string((long long)(retcode)).c_str();
                              OsimLogException(errMsg.data(), __FILE__, __LINE__).throwException();
                        }
                       //create trafodion external table
                       retcode = executeFromMetaContext(statement.data());
                       if(retcode < 0)
                       {
                              CmpCommon::diags()->mergeAfter(*(cliInterface_->getDiagsArea()));
...
void OptimizerSimulator::loadHistograms(const char* histogramPath, NABoolean isHive)
{
   ...
   if(isHive && iterator.entries() > 0)
   {
       ...
       {
            CmpCommon::diags()->mergeAfter(*(cliInterface_->getDiagsArea()));
            NAString errMsg = "create hive stats schema: " ;
            errMsg += std::to_string((long long)(retcode)).c_str();
            OsimLogException(errMsg.data(), __FILE__, __LINE__).throwException();
       }
   }
...
void OptimizerSimulator::readAndSetCQDs()
{
   ...
   if(!isFileExists(logFilePaths_[CQD_DEFAULTSFILE], isDir))
   {
     ...
     snprintf(errMsg, sizeof(errMsg), "Unable to open %s file for reading data.",
                       logFilePaths_[CQD_DEFAULTSFILE]);
     OsimLogException(errMsg, __FILE__, __LINE__).throwException();
   }
   ...
   {
        NAString errMsg = "Error open ";
        errMsg += logFilePaths_[CQD_DEFAULTSFILE];
        OsimLogException(errMsg.data(), __FILE__, __LINE__).throwException();
   } 
...
void OptimizerSimulator::readLogFile_getNodeAndClusterNumbers()
{
  ...
  if(!isFileExists(logFilePaths_[NODE_AND_CLUSTER_NUMBERS],isDir))
  {
    char errMsg[38+OSIM_PATHMAX+1]; // Error errMsg below + filename + '\0'
    snprintf(errMsg, sizeof(errMsg), "Unable to open %s file for reading data.",
                       logFilePaths_[NODE_AND_CLUSTER_NUMBERS]);
    OsimLogException(errMsg, __FILE__, __LINE__).throwException();
  }
...
void OptimizerSimulator::readLogFile_captureSysType()
{
  ...
  if(!isFileExists(logFilePaths_[CAPTURE_SYS_TYPE],isDir))
  {
    ...
    char errMsg[38+OSIM_PATHMAX+1]; // Error errMsg below + filename + '\0'
    snprintf(errMsg, sizeof(errMsg), "Unable to open %s file for reading data.",
                       logFilePaths_[CAPTURE_SYS_TYPE]);
    OsimLogException(errMsg, __FILE__, __LINE__).throwException();
  }
...
void OptimizerSimulator::captureHiveTableStats(HHDFSTableStats* tablestats, const NATable* naTab)
{
     ...
     if(!hashDict_HiveTables_->contains(&qualName))
     {
         ...
         OsimHHDFSStatsBase* root = tablestats->osimSnapShot();
...
void OsimHHDFSFileStats::serializeAttrs(XMLString & xml)
{
    if(NULL == mirror_) 
     ...
     if(hhstats->replication_*hhstats->numBlocks_ > 0)
     {
...
void OsimHHDFSORCFileStats::serializeAttrs(XMLString & xml)
{
    if(NULL == mirror_) 
        return;

     OsimHHDFSFileStats::serializeAttrs(xml);

     HHDFSORCFileStats* hhstats = (HHDFSORCFileStats*)mirror_;
     
     if(hhstats->numOfRows_.entries() > 0)
     {
         xml.append("numOfRows='");
         for(Int32 i = 0; i < hhstats->numOfRows_.entries(); i++)
         {
             xml.append(std::to_string((long long)(hhstats->numOfRows_[i])).c_str()).append('|');
         }
         xml[xml.length()-1]='\'';
         xml.append(' ');
     }
     else
         xml.append("numOfRows='no_entry' ");

     if(hhstats->offsets_.entries() > 0)
     {
         xml.append("offsets='");
         for(Int32 i = 0; i < hhstats->offsets_.entries(); i++)
         {
             xml.append(std::to_string((long long)(hhstats->offsets_[i])).c_str()).append('|');
         }
         xml[xml.length()-1]='\'';
         xml.append(' ');
     }
     else
         xml.append("offsets='no_entry' ");

     if(hhstats->totalBytes_.entries() > 0)
     {
         xml.append("totalBytes='");
         for(Int32 i = 0; i < hhstats->totalBytes_.entries(); i++)
         {
             xml.append(std::to_string((long long)(hhstats->totalBytes_[i])).c_str()).append('|');
         }
         xml[xml.length()-1]='\'';
         xml.append(' ');
    }
    else
         xml.append("totalBytes='no_entry' ");
}
...
NABoolean OsimHHDFSORCFileStats::setValue(HHDFSStatsBase* stats, const char *attrName, const char *attrValue)
{
    if(NULL == stats)
        return FALSE;
...
void OsimHHDFSTableStats::startElement(void *parser, const char *elementName, const char **atts)
{
    if(!strcmp(elementName, TAG_HHDFSLISTPARTSTATS)){
       ...
       OsimLogException("Errors Parsing stats file.", __FILE__, __LINE__).throwException();
}
