...
def run():
    ...
    parser.add_option("--adminuser",  dest="adminuser",    action="store",
            help="The EsgynDB Admin User ID",            metavar="ADMINUSER")
    parser.add_option("--adminpassword",  dest="adminpassword",    action="store",
            help="The password for EsgynDB Admin user",    metavar="ADMINPASSWORD")
    ...
    if not options.adminpassword :
        ...
        while (done==None) :
            options.adminpassword = getpass.getpass(prompt='Please provide the password for EsgynDB Admin User: ')
            if options.adminpassword and len(options.adminpassword)>1: done = True
    ...
    obfustcated_adminpw = encode_pswd(options.adminpassword,dbmgr_path)
    ...
    file_str=file_str.replace('ADMIN_USERID', str(options.adminuser))
    file_str=file_str.replace('ADMIN_PASSWORD', str(obfustcated_adminpw))
