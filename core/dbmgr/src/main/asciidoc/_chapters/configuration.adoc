...
*TSD Host*::
*Only enabled and used in Enterprise Edition*. The hostname or IP address for the node on which TSD is listening. DB Manager uses this property to make a HTTP connection to OpenTSDB to query the timeseries metrics.
*TSD Port*::
*Only enabled and used in Enterprise Edition*. The listen port number for TSD. DB Manager uses this property to make a HTTP connection to OpenTSDB to query the timeseries metrics.
...
In non-enterprise edition, You will need to start EsgynDB Manager as an embedded jetty server using the dbmgr.sh script.
[source,bash,subs="attributes"]
----
$ cd dbmgr-{projectVersion}/bin
./dbmgr.sh start
...
In non-enterprise edition, To stop EsgynDB Manager, run the following command from the bin directory.
----
$ ./dbmgr.sh stop
