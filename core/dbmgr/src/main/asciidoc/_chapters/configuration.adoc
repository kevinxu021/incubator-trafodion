...
** Find the canonical time zone name of the EsgynDB instance. DB Manager displays all date time values in the EsgynDB server local time. You can use the handy script *gettimezone.sh* that is provided in the *dbmgr-{projectVersion}/bin* directory to find the canonical time zone name like *Etc/UTC* or *America/Los_Angeles*.
+
[source,bash,subs="attributes"]
----
...
The canonical time zone name of the EsgynDB instance. DB Manager displays all date time values in the EsgynDB server local time.
The timezone name has to be in the canonical format like *Etc/UTC* or *America/Los_Angeles*. You can use the handy script *gettimezone.sh* to find the canonical time zone name of the instance.
...
* *CMON* runs on the primary node. It is started and managed by the EsgynDB monitor process, so CMON is inherently persistent.
+
...
* *NMON* runs on every node. It is started and managed by the EsgynDB monitor process, so NMON is inherently persistent.
