...
Type the SQL query text in the Query text area. This should a valid EsgynDB query syntax. If not the engine will reject and report an error.
...
==== Explain Plan
You can use the *Explain* button to generate an explain plan for the query. This is helpful for users to analyze the performance cost of the query and allows them to tune it.

==== Control Options
The Control options allow you enter SET statements or Control Query statements that can influence the query plan.
