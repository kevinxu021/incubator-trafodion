...
public class SessionModel {
	...
	static private Map<String, ScriptsInterface> activeScripts = new HashMap<String, ScriptsInterface>();
			...
			synchronized (activeSessions) {
				...
				while (iter.hasNext()) {
					...
					if (o.getValue() instanceof Session) {
						...
						if (Session.isExpired(soc))
							sessionIDsToLogout.add(o.getKey());
						else {
			...
			for (String sessionIDToLogout : sessionIDsToLogout) {
				if (activeSessions.containsKey(sessionIDToLogout)) {
					...
					if (!loggedIn.containsKey(soc.getUsername().toLowerCase())) {
						...
						// delete all sessionObject associated with user to free
						// up memory
	...
	public static void putStatementObject(String key, Object obj) {
		// since only one query could be executed on workbench, so no matter key
		// exist or not. we store it.
	...
	public static Session getSession(HttpServletRequest servletRequest, HttpServletResponse servletResponse)
		...
		if (token != null && !token.equals("")) {
			if (activeSessions.containsKey(token)) {
				...
				if (!Session.isExpired(result)) {
					result.setDate(new DateTime(DateTimeZone.UTC)); // reset
																	// date
	...
	public static void putScriptsObject(String key, ScriptsInterface obj) {
		synchronized (activeScripts) {
			activeScripts.put(key, obj);
		}
	}

	public static ScriptsInterface getScriptsObject(String key) {
		if (activeScripts.containsKey(key)) {
			synchronized (activeScripts) {
				return activeScripts.get(key);
			}
		}
		return null;
	}

	public static void removeScriptsObject(String key) {
		if (activeScripts.containsKey(key)) {
			synchronized (activeScripts) {
