...
public class CPAbort {
	...
	ParseArgs lv_pa = new ParseArgs("CPAbort.log");
	...
	if (clientCache == null) {
        ...
        if (lv_pa.m_verbose) System.out.println("Connection to the monarch distributed system successful");
	    ...
	    if (lv_pa.m_verbose) System.out.println("AbortTransaction"
		...
		if (collector.size() > 0) {
			...
			if (lv_rp_cpr.m_has_exception && (lv_rp_cpr.m_exception != null)) {
			    System.out.println("Abort response exception: "
					       + lv_rp_cpr.m_exception);
			    lv_rp_cpr.m_exception.printStackTrace();
			}
			else {
			    System.out.println("Abort request status code: " + lv_rp_cpr.m_status_code
					       );
			}
		    }
		}
